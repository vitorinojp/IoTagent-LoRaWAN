...
class TtnAppService extends appService.AbstractAppService {
	...
	preProcessMessage(mqttTopic, message) {
		...
		if (splittedMqttTopic.length !== 5) {
			const errorMessage = 'Bad format for a TTN topic. Topic = ' + mqttTopic;
			...
			if (message) {
				...
				if (dataModel === 'application_server_raw_string' && message.uplink_message) {
					this.messageHandler(this, deviceId, deviceEui, {
						rawMessage: encodeURIComponent(JSON.stringify(message.uplink_message))
					});
	...
	observeDevice(devId, devEUI, deviceObject) {
		const mqttTopic = '$share/group/v3/' + this.applicationServer.username + '/devices/' + devId + '/up';
	 ...
	 */
	observeAllDevices() {
		const mqttTopic = '$share/group/v3/' + this.applicationServer.username + '/devices/+/up';
		this.mqttClient.subscribeTopic(mqttTopic);
		winston.info('Mqtt topic subscribed:%s', mqttTopic);
	}
